 ...
 * 
 */
public class CustomSwitchButton extends CompoundButton {
	...
	public CustomSwitchButton(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.customSwitchStyleAttr);
	}
	...
	public CustomSwitchButton(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.CustomSwitch, defStyle, 0);

		mLeftBackground = a
				.getDrawable(R.styleable.CustomSwitch_leftBackground);
		// if (mLeftBackground == null) {
		// mLeftBackground = new ColorDrawable(0xFF11BB33);
		// }
		mRightBackground = a
				.getDrawable(R.styleable.CustomSwitch_rightBackground);
		// if (mRightBackground == null) {
		// mRightBackground = new ColorDrawable(0x993322BB);
		// }

		mOrientation = a.getInteger(R.styleable.CustomSwitch_orientation,
				HORIZONTAL);
		// Log.d(TAG, "mLeftBackground="+mLeftBackground);
		// Log.d(TAG, "mRightBackground="+mRightBackground);
		mThumbDrawable = a.getDrawable(R.styleable.CustomSwitch_thumb);
		mTrackDrawable = a.getDrawable(R.styleable.CustomSwitch_track);
		mTextOn = a.getText(R.styleable.CustomSwitch_textOn);
		mTextOff = a.getText(R.styleable.CustomSwitch_textOff);
		mDrawableOn = a.getDrawable(R.styleable.CustomSwitch_drawableOn);
		mDrawableOff = a.getDrawable(R.styleable.CustomSwitch_drawableOff);
		mPushStyle = a.getBoolean(R.styleable.CustomSwitch_pushStyle, false);
		mTextOnThumb = a
				.getBoolean(R.styleable.CustomSwitch_textOnThumb, false);
		mThumbExtraMovement = a.getDimensionPixelSize(
				R.styleable.CustomSwitch_thumbExtraMovement, 0);
		mThumbTextPadding = a.getDimensionPixelSize(
				R.styleable.CustomSwitch_thumbTextPadding, 0);
		mTrackTextPadding = a.getDimensionPixelSize(
				R.styleable.CustomSwitch_trackTextPadding, 0);
		mSwitchMinWidth = a.getDimensionPixelSize(
				R.styleable.CustomSwitch_switchMinWidth, 0);
		mSwitchMinHeight = a.getDimensionPixelSize(
				R.styleable.CustomSwitch_switchMinHeight, 0);
		mSwitchPadding = a.getDimensionPixelSize(
				R.styleable.CustomSwitch_switchPadding, 0);
		...
		mMaskDrawable = a.getDrawable(R.styleable.CustomSwitch_backgroundMask);
		...
		int appearance = a.getResourceId(
				R.styleable.CustomSwitch_switchTextAppearanceAttrib, 0);
		if (appearance != 0) {
	...
	public void setSwitchTextAppearance(Context context, int resid) {
		TypedArray appearance = context.obtainStyledAttributes(resid,
				R.styleable.customSwitchTextAppearanceAttrib);

		ColorStateList colors;
		int ts;

		colors = appearance
				.getColorStateList(R.styleable.customSwitchTextAppearanceAttrib_textColor);
		if (colors != null) {
			mTextColors = colors;
		} else {
		...
		ts = appearance.getDimensionPixelSize(
				R.styleable.customSwitchTextAppearanceAttrib_textSize, 0);
		if (ts != 0) {
		...
		typefaceIndex = appearance.getInt(
				R.styleable.customSwitchTextAppearanceAttrib_typeface, -1);
		styleIndex = appearance.getInt(
				R.styleable.customSwitchTextAppearanceAttrib_textStyle, -1);
	...
	private Layout makeLayout(CharSequence text) {
		return new StaticLayout(text, mTextPaint,
				(int) android.util.FloatMath.ceil(Layout.getDesiredWidth(text,
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mOrientation == HORIZONTAL) {
			...
			if (mPushStyle) {
				...
				if (rightBitmap != null && this.isChecked()) {
					canvas.save();
					/*
					if (canvas.getClipBounds(canvasClipBounds)) {
				...
				if (leftBitmap != null && !this.isChecked()) {
					canvas.save();
					/*
					if (canvas.getClipBounds(canvasClipBounds)) {
